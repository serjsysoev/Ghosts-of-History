    ...
    fun createOnGlThread(context: Context, depthTextureId: Int = -1) {
        ...
        run {
            val vertexShader = ShaderUtil.loadGLShader(
                    ...
                    context = context,
                    type = GLES20.GL_VERTEX_SHADER,
                    filename = DEPTH_VISUALIZER_VERTEX_SHADER_NAME)
            val fragmentShader = ShaderUtil.loadGLShader(
                    tag = TAG,
                    context = context,
                    type = GLES20.GL_FRAGMENT_SHADER,
                    filename = DEPTH_VISUALIZER_FRAGMENT_SHADER_NAME)
    ...
    fun draw(frame: Frame, debugShowDepthMap: Boolean =  /*debugShowDepthMap=*/false) {
        ...
        if (frame.hasDisplayGeometryChanged()) {
            frame.transformCoordinates2d(
                    /* inputCoordinates = */ Coordinates2d.OPENGL_NORMALIZED_DEVICE_COORDINATES,
                    /* inputVertices2d = */ quadCoords,
                    /* outputCoordinates = */ Coordinates2d.TEXTURE_NORMALIZED,
                    /* outputVertices2d = */ quadTexCoords)
        }
